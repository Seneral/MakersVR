...
static std::chrono::time_point<std::chrono::high_resolution_clock> g_lastReceiveTime;
...
void StartStreaming(ConfiguratorState &state)
{
	...
	state.control.tracking.maxTemporalStaticErrorT = state.config.tracking.maxTemporalStaticErrorT;
	state.control.tracking.maxTemporalStaticErrorR = state.config.tracking.maxTemporalStaticErrorR;
	state.control.tracking.maxTemporalDynamicErrorT = state.config.tracking.maxTemporalDynamicErrorT;
	state.control.tracking.maxTemporalDynamicErrorR = state.config.tracking.maxTemporalDynamicErrorR;
	state.control.tracking.sigmaError = state.config.tracking.sigmaError;
	state.control.calibration.maxRelationCandidateDiffT = state.config.extrinsicCalib.maxRelationCandidateDiffT;
	state.control.calibration.maxRelationCandidateDiffR = state.config.extrinsicCalib.maxRelationCandidateDiffR;
	state.control.calibration.maxOriginCandidateDiffT = state.config.extrinsicCalib.maxOriginCandidateDiffT;
	state.control.calibration.maxOriginCandidateDiffR = state.config.extrinsicCalib.maxOriginCandidateDiffR;
	state.control.calibration.maxPoseMSE = state.config.extrinsicCalib.maxPoseMSE;
	...
	((uint16_t*)setupData)[0] = state.config.mode.cameraResolutionX;
	((uint16_t*)setupData)[1] = state.config.mode.cameraResolutionY;
	((uint16_t*)setupData)[2] = state.config.mode.cameraFramerate;
	((uint16_t*)setupData)[3] = state.config.mode.cameraShutterSpeed;
	setupData[8] = state.config.mode.cameraGain;
	setupData[9] = state.config.mode.cameraAbsThreshold;
	setupData[10] = state.config.mode.cameraEdgeThreshold;
...
void StartTesting(ConfiguratorState &state)
{
	...
	for (int i = 0; i < state.config.testing.trackingMarkers.size(); i++)
	{
		...
		{ // Determine offset of calibrated marker template compared to ground truth
			...
			auto pose = detectMarker3D(&*existingCalib, pointCloud, {}, pointCloud.size(), state.control.tracking.sigmaError*2, false);
			if (std::get<2>(pose) > 0)
...
//		if (state->updateTestThread)
		{
			...
			{ // Generate pose facing camera withing range min,max
				...
				auto genExp = [](float pow){
					...
					float exp = std::pow(std::abs(rnd), pow);
			...
			{
				if (control->phase == PHASE_Calibration_Intrinsic  && state->control.calibration.phaseFocus >= 0)
				{ // Marker in front of cameras
					Camera *camera = &control->cameras[state->control.calibration.phaseFocus].testing.camera;
				...
				else if (control->phase == PHASE_Calibration_Extrinsic && state->control.calibration.phaseFocus >= 0)
				{ // Marker facing two cameras
					Camera *cameraA = &control->cameras[control->calibration.relations[state->control.calibration.phaseFocus].camA].testing.camera;
					Camera *cameraB = &control->cameras[control->calibration.relations[state->control.calibration.phaseFocus].camB].testing.camera;
					// Generate initial pose
					if (control->testing.targetPose.translation().norm() == 0)
			...
			{		
				...
				if (control->phase == PHASE_Calibration_Extrinsic || control->phase == PHASE_Calibration_Room)
				{
					for (int m = 0; m < control->cameras.size(); m++)
					{
						CameraState *camState = &control->cameras[m];
						...
						{ // Found marker pose using CV, check against GT (ground truth)
							...
							wxLogMessage(wxT("Cam %d, Pose %d: MSE %.4fmpx, Error (%.4fmm, %.4f\u00B0)"), 
								m, p, camState->extrinsic.posesMSE[0]*1000, poseError.first*10, poseError.second);
						}
					}
				}
			}
				...
				if (control->phase == PHASE_Tracking)
				{
					...
					{ // Determine pose error
						for (int i = 0; i < control->tracking.poses3D.size(); i++)
						{
							...
							wxLogMessage(wxT("Pose %d, Points: %d, MSE: %.4fmm, Error: (%.4fmm, %.4f\u00B0)"),
								i, poseMSE.second, poseMSE.first*10, poseError.first*10, poseError.second);
							if (poseError.first > 4 || poseError.second > 10)
							{ // Pose is wrong
...
static void onControlResponse(uint8_t request, uint16_t value, uint16_t index, uint8_t *data, int length, void *userData)
{
	...
	{ // Setup Response
		...
		bool setupChanged = state->cameraMapping.size() != data[0];
		state->cameraCount = 0;
...
static void HandleBlobPacket(ConfiguratorState &state, uint8_t *data, int length)
{
	...
	if (blobCount > 0)
	{
		...
		data[usbHeaderSize] = 0;
		wxLogMessage(wxT("Blobs IN (%d) %s %d Blobs: %s"), length, (char*)data, blobCount, blobBuf.str());
	}
#endif
}
