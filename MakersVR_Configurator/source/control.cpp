...
static void (*OnUpdateStatus)(int);
...
static void ClearCalibrationIntrinsicItem(ControlState *state, int index)
{
	CameraState *cam = &state->cameras[index];
	// Clear potentially running calibration
...
static void InitCalibrationIntrinsic(ControlState *state)
{
	...
	{
		ClearCalibrationIntrinsicItem(state, m);
	}
}
...
static void UpdateCalibrationIntrinsic(ControlState *state)
{
	...
	for (int m = 0; m < state->cameras.size(); m++)
	{
		...
		if (cam->intrinsic.calibrationTimeout > 0)
		{ // Don't take samples too fast
			cam->intrinsic.calibrationTimeout--;
			continue;
		}

		if (!cam->intrinsic.calibrationRunning && m == state->calibration.phaseFocus)
		{ // No updates while calibration is running
			...
			if (cnt > prevCnt)
				cam->intrinsic.calibrationTimeout = 10;
			...
			if (prevCnt < cnt)
				if (OnUpdateStatus) OnUpdateStatus(m);
		}
		...
		{ // Finished last calibration round
			...
			wxLogMessage("Cam %d calibration: %d points, FoV (%f, %f), Distortions (%f, %f, %f, %f, %f)", 
				m, (int)cam->intrinsic.calibrationSelection.size(), cam->camera.fovH, cam->camera.fovV, 
				cam->camera.distortion.k1, cam->camera.distortion.k2, cam->camera.distortion.p1, cam->camera.distortion.p2, cam->camera.distortion.k3);
...
static void ThreadCalibrationIntrinsic(ControlState *state, CameraState *cam)
{
	...
	// Take control of temporary data
	Camera camCpy = cam->camera;
	std::vector<Marker2D> selection = cam->intrinsic.calibrationSelection; // True copy since original list needed
	std::vector<double> errors; // Switch only since content only needed when finished and can be discarded if never finished
	errors.swap(cam->intrinsic.calibrationMarkerErrors);

	// Perform calibration on temporary data
	calculateIntrinsicCalibration(camCpy, selection, *state->calibration.markerTemplate2D, errors);

	if (cam->intrinsic.calibrationRunning && !cam->intrinsic.calibrationStopped && cam->intrinsic.calibrationThread->get_id() == std::this_thread::get_id())
	{ // If result is still expected, reinstate temporary data and results
		cam->camera = camCpy;
		errors.swap(cam->intrinsic.calibrationMarkerErrors);
		cam->intrinsic.calibrationMarkerCount = selection.size();
...
static void ClearCalibrationExtrinsicItem(ControlState *state, int index)
{
	state->calibration.relations[index].candidates.clear();
}
...
{
//	for (int r = 0; r < state->calibration.relations.size(); r++)
	int r = state->calibration.phaseFocus;
	if (r >= 0)
	{
		CameraRelation *rel = &state->calibration.relations[r];
		...
		{
			if (camA->extrinsic.posesMSE[a] > state->calibration.maxPoseMSE) continue;
			const Eigen::Isometry3f poseA = camA->extrinsic.poses[a];
			for (int b = 0; b < camB->extrinsic.poses.size(); b++)
			{
				if (camB->extrinsic.posesMSE[b] > state->calibration.maxPoseMSE) continue;
				...
				int candidateInd = AddTransformToCandidates(rel->candidates, transAB, weight, state->calibration.maxRelationCandidateDiffT, state->calibration.maxRelationCandidateDiffR);

				if (state->testing.isTesting)
				{ // Debug
					// Calculate error between data point / candidate and ground truth
					Eigen::Isometry3f transABGT = camA->testing.camera.transform.inverse() * camB->testing.camera.transform;
					...
					// Debug
					wxLogMessage(wxT("Cam Rel %d-%d: (%.4fmm, %.4f\u00B0) => (%.4fmm, %.4f\u00B0, %d, %d, %.4f)"),
						rel->camA, rel->camB, poseError.first*10, poseError.second, candError.first*10, 
						candError.second, candidateInd, (int)candidate->datapoints.size(), candidate->weight);
				}

				if (OnUpdateStatus) OnUpdateStatus(r);
			}	
		}
	}
}
...
static void FinalizeCalibrationRoom(ControlState *state)
{
	...
	if (state->testing.isTesting)
	{
		...
		{ // Debug
			...
			wxLogMessage(wxT("Camera %d error: (%.4fmm, %.4f\u00B0)"), m, tError*10, rError);
		}
	}
}
...
static void UpdateCalibrationRoom(ControlState *state)
{
	for (int m = 0; m < state->cameras.size(); m++)
	{
		...
		{
			if (cam->extrinsic.posesMSE[p] > state->calibration.maxPoseMSE) continue;
			...
			weight = 1000.0f/(weight*weight);
			// Add to suitable candidate
			int candidateInd = AddTransformToCandidates(cam->extrinsic.originCandidates, pose, weight, state->calibration.maxOriginCandidateDiffT, state->calibration.maxOriginCandidateDiffR);

			if (state->testing.isTesting)
			{ // Debug
				// Calculate error between data point / candidate and ground truth
				TransformCandidate *candidate = &cam->extrinsic.originCandidates[candidateInd];
				std::pair<float,float> poseError = calculatePoseError(cam->testing.camera.transform.inverse(), cam->extrinsic.poses[p]);
				std::pair<float,float> candError = calculatePoseError(cam->testing.camera.transform.inverse(), candidate->transform);
				// Debug
				wxLogMessage(wxT("Cam %d origin: (%.4fmm, %.4f\u00B0) => (%.4fmm, %.4f\u00B0, %d, %d, %.4f)"),
					m, poseError.first*10, poseError.second, candError.first*10, candError.second,
					candidateInd, (int)candidate->datapoints.size(), candidate->weight);
			}
		}
	}
}
...
static void UpdateCalibrationMarker(ControlState *state)
{
	...
	else if (state->tracking.nonconflictedCount > 3)
	{ // Iteratively add and improve marker points
		...
		{ // Debug
			...
			wxLogMessage(wxT("Ground truth error (%.4fmm, %.4f\u00B0)"), poseError.first*10, poseError.second);
		}
		...
		{ // Debug
			...
			wxLogMessage(wxT("Marker points: %d candidates; %d matched, %d missing, %d excess; MSE %.4fmpx, Error (%.4fmm, %.4f\u00B0)"),
				(int)checkCandidates.size(), checkCandPtCnt, (int)(pointCloud.size() - checkCandPtCnt), 
				(int)(state->markerCalib.markerPoints.size() - checkCandPtCnt), std::get<1>(checkCand)*1000, poseError.first*10, poseError.second);
...
static void TrackMarkers2D(ControlState *state)
{
	for (int m = 0; m < state->cameras.size(); m++)
	{
		...
		matchTrackedPoses(cam->extrinsic.poses, cam->extrinsic.lastPoses, cam->extrinsic.poseDiffPos, cam->extrinsic.poseDiffRot, cam->extrinsic.posesMatch, state->tracking.maxTemporalStaticErrorT, state->tracking.maxTemporalStaticErrorR, state->tracking.maxTemporalDynamicErrorT, state->tracking.maxTemporalDynamicErrorR);
...
//					wxLogMessage("Cam %d, Pose %d (%d):  Flipped, new MSE: %f! Diffs exp: %f, act: %f, flp: %f", m, p, l, flippedMSE, expectedAngleDiff/PI*180, curAngleDiff/PI*180, flippedAngleDiff/PI*180);
		...
		for (int p = 0; p < cam->extrinsic.poses.size(); p++)
		{
			if (cam->extrinsic.posesMSE[p] > state->calibration.maxPoseMSE)
			{
...
std::string GetItemStatus(ControlState *state, int index)
{
	...
	{
		CameraState *cam = &state->cameras[index];
		if (cam->intrinsic.calibrationMarkerCount > 0)
		{
			return wxString::Format("%d markers;%s FoV (%.3f, %.3f), Dist (%.3f, %.3f, %.3f, %.3f, %.3f)", 
				(int)cam->intrinsic.calibrationSelection.size(),
				cam->intrinsic.calibrationRunning? " Calibrating..." : "",
				cam->camera.fovH, cam->camera.fovV, 
				cam->camera.distortion.k1, cam->camera.distortion.k2,
				cam->camera.distortion.p1, cam->camera.distortion.p2,
				cam->camera.distortion.k3).ToStdString();
		}
		else
		{
			return wxString::Format("%d markers%s", 
				(int)cam->intrinsic.calibrationSelection.size(),
				cam->intrinsic.calibrationRunning? "; Calibrating..." : "").ToStdString();
		}
	}
	else if (state->phase == PHASE_Calibration_Extrinsic)
	{
		CameraRelation *rel = &state->calibration.relations[index];
		CameraState *camA = &state->cameras[rel->camA];
		CameraState *camB = &state->cameras[rel->camB];
		TransformSample bestCandidate = chooseBestTransformCandidate(rel->candidates);
		if (state->testing.isTesting)
		{ // Debug: Calculate error between best candidate and ground truth
			Eigen::Isometry3f transABGT = camA->testing.camera.transform.inverse() * camB->testing.camera.transform;
			std::pair<float,float> poseError = calculatePoseError(transABGT, bestCandidate.transform);
			return wxString::Format(wxT("%d samples, %.4f weight, %.4f translational std dev, %.4f rotational std dev, error (%.4fmm, %.4f)"),
				bestCandidate.sampleCount, bestCandidate.weight, bestCandidate.stdDevT, bestCandidate.stdDevR, poseError.first*10, poseError.second).ToStdString();
		}
		else
		{
			return wxString::Format(wxT("%d samples, %.4f weight, %.4f translational std dev, %.4f rotational std dev"),
				bestCandidate.sampleCount, bestCandidate.weight, bestCandidate.stdDevT, bestCandidate.stdDevR).ToStdString();
		}
	}
	else
...
void ResetItem(ControlState *state, int index)
{
	if (state->phase == PHASE_Calibration_Intrinsic)
	{
		ClearCalibrationIntrinsicItem(state, index);
	}
	else if (state->phase == PHASE_Calibration_Extrinsic)
	{
		ClearCalibrationExtrinsicItem(state, index);
	}
}